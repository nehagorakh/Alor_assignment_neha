...
public class HotelServiceImpl implements HotelService{
	...
	public List<HotelResponseBean> getHotels(String city, Integer numberOfRooms, String date, Integer wifi, Integer resturant, Integer ac, Integer mealsIncluded,
		...
		Map<Integer, List<HotelEntity>> hotelMap = hotelRepository.findByIdIn(hotelIds).stream()
				.collect(Collectors.groupingBy(HotelEntity::getId));
		...
		List<HotelResponseBean> hotelResponseBeanListWithSomeBookedRooms = getHotelWithSomeBookedAndAvailableRooms(
				hotelRoomBookings, hotelMap, hotelFacilityMap, hotelRatingMapGroupByRating);
		...
		List<HotelResponseBean> hotelResponseBeanListWithAllAvailableRooms = getHotelWithAllAvailbaleRooms(hotelIds,
	...
	private List<HotelResponseBean> getHotelWithAllAvailbaleRooms(Set<Integer> hotelIds,
		...
		for (int hoteId : hotelIds) {
			...
			if (hotelRatingMapGroupByRating.size() > 0)
				hotelResponseBean1.setAverageRating((int) hotelRatingMapGroupByRating.get(hoteId).stream()
	...
	private List<HotelResponseBean> getHotelWithSomeBookedAndAvailableRooms(List<HotelRoomBooking> hotelRoomBookings,
		...
		if (hotelRoomBookings.size() > 0) {
			...
			for (Map.Entry<Integer, List<HotelRoomBooking>> entry : hotelRoomBookingMapListGroupByHotelId.entrySet()) {
				...
				BeanUtils.copyProperties(hotelFacilityMap.get(hotelResponseBean.getHotelId()).get(0),
						hotelResponseBean);

				// getting and setting available tooms
				hotelResponseBean.setAvailableRooms(hotelResponseBean.getNumberOfRooms()
						- (entry.getValue().stream().mapToInt(hrb -> hrb.getNumberOfRooms()).sum()));

				// getting and setting rating
				if (hotelRatingMapGroupByRating.size() > 0)
					hotelResponseBean.setAverageRating((int) hotelRatingMapGroupByRating.get(entry.getKey()).stream()
							.mapToDouble(r -> r.getRating()).average().getAsDouble());
